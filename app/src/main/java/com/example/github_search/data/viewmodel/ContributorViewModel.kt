...
class ContributorViewModel @Inject constructor(
    ...
    var contributorsRepo = MutableLiveData<Resource<List<Item>>>()

    fun getContributersList(name: String, page: String, per_page: String) {
        viewModelScope.launch {
            withContext(Dispatchers.Default) {
                if (githubsearch.applicationContext.hasInternetConnection()) {
                    contributorsRepo.postValue(Resource.Loading())
                    contributorsRepo.postValue(
                        handleApiResponse(
                            repository.contribursRepoReq(
                                name,
                                page,
                                per_page
                            )
    ...
    private fun <T> handleApiResponse(response: Response<List<T>>?): Resource<List<T>> {
